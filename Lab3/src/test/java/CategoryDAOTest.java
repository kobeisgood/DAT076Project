...
@Deployment
    ...
    public void categoryTest() {
        ...
        assertEquals(1,categories.get(1).getTransactions().size());
        
        
        int sum = 0;
        for(Transactions t : categories.get(0).getTransactions()){
            sum += t.getAmount();   
        }
        assertEquals(-1100,sum);
        
        sum = 0;
        for(Transactions t : categories.get(3).getTransactions()){
            sum += t.getAmount();   
        }
